...
async function updatePredictions() {
  ...
  const dotprod = Math.sqrt(predictions[0]*predictions[0]+predictions[1]*predictions[1]+
                            predictions[2]*predictions[2]+predictions[3]*predictions[3]+
                            predictions[4]*predictions[4]+predictions[5]*predictions[5])
  const maxPrediction = Math.max(...predictions);

  for (let i = 0; i < predictions.length; i++) {
    const element = document.getElementById(`prediction-${i}`);
    element.children[0].children[0].style.height = `${predictions[i]*100/dotprod}%`;
    element.className =
...
loadingModelPromise.then(() => {
  ...
  ctx.clearRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
  ctx.fillText("Draw a number here!", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2);
})